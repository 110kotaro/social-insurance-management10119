...
#### Firebase Console設定（実装済み）

**アクション URL設定**:
- Firebase Console > 認証 > テンプレート > アクション URL
- 設定値: `https://kensyu10119.web.app/password-setup`
- **重要**: この設定は「メール テンプレートすべてに適用されます」
  - パスワードリセットメール（`mode=resetPassword`）
  - メール認証メール（`mode=verifyEmail`）
  - その他のアクションコードメール
- すべてのメールテンプレートで同じURLが使用されるため、コード側で`mode`パラメータに基づいて処理を分岐する必要がある
...
#### 1.2 認証基盤
- [x] Authentication設定（メール/パスワード）
  - Firebase Console > Authentication > Sign-in method でメール/パスワードを有効化
  - Firebase Console > 認証 > テンプレート > アクション URL を設定: `https://kensyu10119.web.app/password-setup`
  - **注意**: アクション URLは全メールテンプレートに適用されるため、コード側で`mode`パラメータによる分岐処理が必要
- [x] 認証サービス実装
  - 既存アプリとは独立した認証フロー
  - [x] 初回アカウント作成（メール認証）
    - `register()`: メール/パスワードでアカウント作成
    - `sendEmailVerification()`: メール認証リンク送信（`ActionCodeSettings`で`/email-verification`にリダイレクト）
    - Firestoreの`socialInsurance_users`コレクションに`role: 'owner'`でユーザー情報を保存
  - [x] 社員招待（パスワード設定）
    - `inviteEmployee()`: Firebase Authenticationでアカウント作成（一時パスワード）
    - `sendPasswordResetEmail()`: パスワード設定リンク送信（`ActionCodeSettings`で`/password-setup`にリダイレクト）
    - Firestoreの`socialInsurance_users`コレクションに`role: 'employee'`でユーザー情報を保存
  - [x] ログイン（メールアドレス + パスワード）
    - `login()`: Firebase Authenticationで認証後、Firestoreの`socialInsurance_users`コレクションを確認
    - ユーザーが存在しない場合はエラー（既存アプリのユーザーと区別）
    - 最終ログイン時刻を更新
- [x] 認証ガード実装
  - [x] `AuthGuard`: 認証済みチェック
    - `currentUser`が存在し、`isActive: true`の場合にアクセス許可
    - メール認証未完了かつ組織未作成の場合は`/email-verification`にリダイレクト
    - ダッシュボードアクセス時もメール認証チェック
...
- [x] メール認証画面
  - メール認証リンククリック後の処理
    - URLパラメータから`oobCode`と`mode`を取得
    - `mode=verifyEmail`の場合: `applyActionCode()`でメール認証を完了
    - 認証完了後、認証状態を再読み込みして「認証しました」画面を表示
    - 組織が作成済みの場合はダッシュボード、未作成の場合は組織作成画面に遷移
  - 認証メール再送信機能
- [x] パスワード設定画面（社員招待用）
  - URLパラメータから`oobCode`と`mode`を取得
  - `mode=resetPassword`の場合: パスワード設定処理を実行
    - `verifyPasswordResetCode()`でリセットコードを検証してメールアドレスを取得
    - `confirmPasswordReset()`でパスワードをリセット
    - `signInWithEmailAndPassword()`で自動ログイン
    - Firestoreの`socialInsurance_users`コレクションにデータを作成/更新（`employees`コレクションから`employeeId`と`organizationId`を取得）
    - `authService.login()`でFirestoreのチェックと最終ログイン時刻を更新
    - ダッシュボードに遷移
  - `mode=verifyEmail`の場合: `/email-verification`にリダイレクト（Firebase Consoleの設定で全テンプレートが`/password-setup`にリダイレクトされるため）
- [x] パスワードリセット機能（既存ユーザー向け）
  - `auth.service.ts`の`sendPasswordReset()`で実装
  - `ActionCodeSettings`で`/login`にリダイレクト設定
  - Firebase Consoleの「アクション URL」設定により、実際のリダイレクト先は`/password-setup`になる
  - パスワード設定画面で処理（社員招待と同じフロー）

#### 認証フローの実装詳細

**Firebase Console設定とコード側の処理**:
1. Firebase Consoleの「アクション URL」を`https://kensyu10119.web.app/password-setup`に設定
   - この設定は全メールテンプレート（パスワードリセット、メール認証など）に適用される
   - `handleCodeInApp: false`でも、この設定が優先される

2. `password-setup.component.ts`での処理:
   - URLパラメータから`oobCode`と`mode`を取得
   - `mode=resetPassword`: パスワード設定処理を実行
   - `mode=verifyEmail`: `/email-verification`にリダイレクト（クエリパラメータも引き継ぐ）

3. `email-verification.component.ts`での処理:
   - URLパラメータから`oobCode`と`mode`を取得
   - `mode=verifyEmail`の場合: `applyActionCode()`でメール認証を完了
   - 認証完了後、`reloadCurrentUser()`で認証状態を再読み込み
   - `checkEmailVerification()`で認証状態を確認して「認証しました」画面を表示

**パスワード設定後の処理**:
- `confirmPasswordReset()`でパスワードをリセット
- `signInWithEmailAndPassword()`で自動ログイン
- `employees`コレクションから`employeeId`と`organizationId`を取得
- Firestoreの`socialInsurance_users`コレクションにデータを作成/更新
- `authService.login()`でFirestoreのチェックと最終ログイン時刻を更新
- ダッシュボードに遷移

**注意事項**:
- Firebase Consoleの「アクション URL」設定は全テンプレートに適用されるため、コード側で`mode`パラメータによる分岐処理が必須
- `handleCodeInApp: false`でも、Firebase Consoleの設定が優先される
- メール認証リンクも`/password-setup`にリダイレクトされるため、`password-setup.component.ts`で`mode=verifyEmail`を検出して`/email-verification`にリダイレクトする必要がある
...
#### 2.1 メール認証フロー
- [x] メール認証送信機能
  - `auth.service.ts`の`register()`と`resendEmailVerification()`で実装
  - `ActionCodeSettings`で`/email-verification`にリダイレクト設定
  - Firebase Consoleの「アクション URL」設定により、実際のリダイレクト先は`/password-setup`になる
- [x] 認証リンククリック後の処理
  - `password-setup.component.ts`で`mode=verifyEmail`を検出して`/email-verification`にリダイレクト
  - `email-verification.component.ts`で`applyActionCode()`を実行してメール認証を完了
  - 認証完了後、認証状態を再読み込みして「認証しました」画面を表示
- [x] 認証状態の管理
  - `onAuthStateChanged`で認証状態を監視
  - Firestoreの`socialInsurance_users`コレクションとFirebase Authenticationの状態を同期
  - `reloadCurrentUser()`で最新の認証状態を取得
...
- [x] CSV/Excel一括インポート
  ...
  - [x] ヘッダー行の解析機能（ヘッダー行から列インデックスをマッピング）
  - [x] 「被保険者整理番号」「基礎年金番号」をヘッダー行から探してマッピング
  - [x] プレビュー表示（全列表示、テキスト折り返しなし、横スクロール）
  - [x] エラーチェック（必須項目、データ形式、重複）
  - [x] サンプルデータ表示（全21列 + 権限列）
  - [x] サンプルデータのCSVテキストとプレビューテーブルのラベルを「被保険者整理番号」「基礎年金番号」に変更
  - [x] 一括登録実行
...
- [x] **事業所番号の表示**
  - [x] 申請詳細画面と内容確認画面で事業所番号が正しく表示されるように修正
  - [x] `formatInsuranceAcquisitionData`、`formatInsuranceLossData`、`formatAddressChangeData`、`formatNameChangeData`メソッドに事業所番号の表示処理を追加
...
- [x] **内容確認ステップ（入力フォームの前）**
  ...
  - [x] **申請作成・更新時の確認ダイアログ**
    - [x] 「申請を作成」ボタン操作時に「この申請を作成しますか？作成後は編集できなくなります。」という確認ダイアログを表示
    - [x] 「申請を更新」ボタン操作時に「この申請を更新しますか？更新後は編集できなくなります。」という確認ダイアログを表示
    - [x] 「下書きとして作成」「下書きを更新」ボタンでは確認ダイアログを表示しない（編集可能のため）
- [x] **入力フォーム（編集用）**
  - [x] 被保険者資格取得届フォームの実装
  - [x] 組織情報から提出者情報の自動設定
...
- [x] 申請作成・削除（基本実装完了）
- [x] 申請編集機能
  - [x] **申請編集時の被保険者情報の社員選択機能改善（修正16）**
    - [x] 既存の被保険者情報は社員選択を無効化（固定表示）
    - [x] 選択済み社員名を表示（ドロップダウンではなくテキスト表示）
    - [x] 新規追加時のみ社員選択可能
    - [x] 社員を変更する場合は、既存の被保険者情報を削除して新規追加が必要
  - [x] **被扶養者異動届（外部申請）に提出日記入欄追加（修正16）**
    - [x] 事業主等受付年月日と被保険者情報の間に提出日フィールドを追加
    - [x] `Application.submissionDate`から取得して年号形式で自動転記（編集可能）
  - [x] **申請のCSV出力に出力年月日を追加（修正16）**
    - [x] CSV出力時に現在の日時を「出力年月日」として追加
  - [x] **報酬月額変更届の判定機能強化（修正16）**
    - [x] 判定期間の3か月に基礎日数不足がある場合は申請対象外とする判定を追加
    - [x] 2等級上昇かつ平均額減少、または2等級下降かつ平均額上昇の場合は申請対象外とする判定を追加
    - [x] `standard-reward-calculation.service.ts`の`calculateMonthlyChange`に判定ロジックを追加
    - [x] `notification.service.ts`の`checkAndSendMonthlyChangeReminders`にも同様の判定を追加
  - [x] **申請一覧の提出期限表示を法定期限・管理者設定期限に合わせる（修正16）**
    - [x] 申請作成時：外部申請は法定期限を優先設定（法定期限がない場合のみ管理者設定期限）
    - [x] 申請作成時：内部申請は管理者設定期限（`internalDeadlineDays`）を使用
    - [x] 申請一覧表示：外部申請は法定期限を計算して表示（法定期限がない場合のみ`application.deadline`を表示）
    - [x] 申請一覧表示：内部申請は`application.deadline`（管理者設定期限）を表示
    - [x] `deadline-calculation.service.ts`の非同期メソッド呼び出しに`await`を追加（`calculateInsuranceAcquisitionDeadline`、`calculateInsuranceLossDeadline`、`calculateBonusPaymentDeadline`）
    - [x] `application-list.component.ts`の`ngOnInit`で`loadOrganization`の完了を待ってから`loadApplications`を実行するように修正
  - [x] **申請一覧のフィルタリングを社員から申請者に変更（修正16）**
    - [x] フィルタリングロジックを`application.employeeId`（申請者）に変更
    - [x] UIラベルを「社員」から「申請者」に変更
  - [ ] **通知送信の問題対応（修正16）**
    - [ ] ダッシュボードからの手動通知送信で期限超過通知が送信されない問題を調査・修正
    - [ ] 通知送信時の期限計算ロジックを確認・修正
  - [x] **社員情報・組織情報・申請情報の拡張（修正17）**
    - [x] **社員情報の拡張**
      - [x] 保険適用開始日を入社日から自動設定（編集可能）
      - [x] 社員編集時の変更履歴・ログ機能追加
      - [x] 社員にファイルを添付可能にする機能
      - [x] 健康保険被保険者番号のラベルを「被保険者整理番号（健康保険）」に変更
      - [x] 厚生年金被保険者番号のラベルを「基礎年金番号」に変更
      - [x] 社員住所のカナ入力欄追加（社員作成・編集画面）
      - [x] 申請作成時に基礎年金番号を自動転記
    - [x] **組織情報の拡張**
      - [x] 組織設定画面に事業主氏名入力欄追加
      - [x] 組織設定画面に電話番号入力欄追加（既存）
    - [x] **申請情報の拡張**
      - [x] 申請作成時に事業主氏名と電話番号を自動転記
      - [x] 申請詳細画面に事業主氏名を表示
      - [x] 資格取得届で資格取得年月日を入社日から自動転記
      - [x] 申請作成・編集時に住所カナを自動転記
      - [x] 申請作成・編集時に基礎年金番号を自動転記（マイナンバーがあっても設定）
    - [x] **申請編集時の社員選択機能改善**
      - [x] 資格取得届の既存データ読み込み時に`employeeId`を逆引きして設定
      - [x] `loadEmployees`を`loadOrganization`より前に実行して、`populateFormWithExistingData`実行時に社員一覧が読み込まれている状態にする
- [x] **申請詳細画面のCSV出力機能**
  - [x] 外部申請の詳細画面でCSV出力ボタンを追加
  - [x] 申請内容をCSV形式で出力
  - [x] CSV出力時に出力年月日を追加
- [x] **保険料計算ページ関連の改善（修正18）**
  - [x] **月表示の変更**
    - [x] 月表示を「月分」に変更（計算一覧画面、計算詳細画面、賞与計算詳細画面）
  - [x] **賞与計算の標準賞与額上限処理**
    - [x] 標準賞与額が150万円を超える場合、厚生年金保険料計算時に150万円を上限として計算
    - [x] 4月～翌3月の標準賞与額累計が573万円を超える場合の処理を実装
      - [x] 計算対象月の前月までの標準賞与額累計を計算
      - [x] 累計が573万円を超える場合は、使用する標準賞与額を調整（例: 6月に300万、翌1月に300万の場合、翌1月は273万円を使用）
  - [x] **遡及控除額の合計金額への反映**
    - [x] 遡及控除額を含めた合計保険料、会社負担額、従業員負担額を計算するメソッドを追加
    - [x] 計算詳細画面と賞与計算詳細画面で、遡及控除額を含めた合計金額を表示
    - [x] 遡及控除が適用されている場合に「（遡及控除額含む）」の表示を追加
...
**計算ロジック:**
- [x] **申請承認済みの場合**
  - [x] **新ルール（条件1・条件2を満たす場合のみ免除）:**
    - [x] 条件1: 休職開始日が含まれる月 = 休職終了日の翌日が含まれる月 が同じ月であること
    - [x] 条件2: 条件1で一致した月内で14日以上休職していること
    - [x] 条件1と条件2を両方満たす場合のみ、その月の保険料を全額免除（計算をスキップして全額0を返す）
    - [x] 条件1または条件2を満たさない場合は、既存ロジックを適用（休職開始日を含む月～休職終了日の翌日を含む月の前月まで全額免除）
  - [x] 計算をスキップして全額0の計算結果を返す
  - [x] 計算結果に免除理由を記録（`notes`フィールドに記録）
  - [x] 月次計算（`calculateEmployeePremium`）と賞与計算（`calculateEmployeeBonusPremium`）の両方に実装
...
- [x] **賞与支払日の設定機能（賞与支払届の期限計算用）**
  - [x] `BonusData`モデルに`bonusPaymentDate?: Date | Timestamp`フィールドを追加
  - [x] 給与情報入力画面に賞与支払日入力欄を追加（賞与額の横に配置）
  - [x] 日付入力欄（`input type="date"`）を使用して支払日を入力
  - [x] `BonusDataService.saveBonusData()`で支払日も保存できるように修正
  - [x] Excel/CSVインポート・エクスポート機能に賞与支払日を追加
  - [x] 賞与支払届の期限計算ロジックを修正
    - [x] `DeadlineCalculationService.calculateBonusPaymentDeadline()`を修正
    - [x] 申請データから`commonPaymentDate`（フォームデータ形式）または`commonBonusPaymentDate`（Date形式）を取得
    - [x] 取得できない場合、該当社員の`BonusData`から支払日を取得して期限を計算
  - [x] 申請データのキー名を統一（`commonPaymentDate`と`commonBonusPaymentDate`の整合性を確保）
    - [x] 申請作成・編集時に`commonPaymentDate`を`commonBonusPaymentDate`（Date形式）に変換して保存
    - [x] 申請編集時のデータ読み込み時に`commonBonusPaymentDate`を`commonPaymentDate`（フォームデータ形式）に変換
...
- [x] **賞与計算ロジックの実装**
  ...
  - [x] **賞与計算の年4回以上の場合の注意書き機能**
    - [x] 判定期間（7月1日～翌6月30日）内の賞与計算回数をチェック
    - [x] 年4回以上の賞与の場合、通常通り計算を実行
    - [x] 計算結果の`notes`フィールドに注意書きを追加：「来年度に賞与が4回以上給付することが確定している場合は、必要な手続きを行ってください。」
    - [x] 計算結果詳細画面に`notes`フィールドを表示（備考・注意事項として表示）
...
- [x] **賞与計算詳細コンポーネントの実装**
  ...
  - [x] 備考・注意事項の表示（`notes`フィールド、改行を保持して表示）
  - [x] 差額情報の表示（再計算時）
  - [x] 遡及控除情報の表示
  - [x] 再計算履歴の表示
...
#### 【修正21】一般社員画面のダッシュボード
- [x] **実装完了**
  - [x] 自分の申請状況：申請種別とそれぞれのステータスを表示（直近5件）（自分の内部申請）
  - [x] 申請作成のショートカット（内部申請のみ：被扶養者（異動）届、氏名変更届、住所変更届）
  - [x] データ読み込みのタイミング修正（組織情報の読み込み完了を待ってからダッシュボードデータを読み込む）

#### 【修正22】オーナー権限での外部申請作成・編集対応
- [x] **実装完了**
  - [x] `Application`型の`employeeId`をオプショナルに変更（外部申請の場合、オーナーが作成する場合はnullの可能性がある）
  - [x] 申請作成時の`employeeId`チェックを修正（外部申請の場合は`employeeId`がnullでも作成可能、内部申請の場合は必須）
  - [x] 申請編集時の`employeeId`チェックを修正（外部申請の場合は`employeeId`がnullでも編集可能、内部申請の場合は必須）
  - [x] 期限計算時の`employeeId`がnullの場合の処理を追加
  - [x] 社員情報取得時の`employeeId`がundefinedの場合の処理を追加

#### 【修正23】メール認証チェックの追加
- [x] **実装完了**
  - [x] `authGuard`にメール認証チェックを追加（組織作成画面とセットアップ画面へのアクセス時にメール認証済みかチェック）
  - [x] `onAuthStateChanged`の初回ログイン処理にメール認証状態の同期を追加
  - [x] `authGuard`にメール認証未完了かつ組織未作成のユーザーを`/email-verification`にリダイレクトする処理を追加（ログイン後のリダイレクト）
    - 社員は`organizationId`が既に設定されているため、このチェックをスキップされる（影響なし）
    - `email-verification`画面へのアクセスは許可（無限ループを防ぐ）

#### 【修正24】Storageセキュリティルールの更新
- [x] **実装完了**
  - [x] 申請添付書類と社員ファイルでExcel/Word形式も許可（アプリ側の設定に合わせて更新）
  - [x] 許可形式：PDF、画像（jpg/jpeg/png/gif/bmp/webp）、Excel（xlsx/xls）、Word（docx/doc）

#### 【修正25】クライアント側ファイルバリデーションの追加
- [x] **実装完了**
  - [x] 申請作成画面にファイル形式・サイズのバリデーションを追加（`application-create.component.ts`）
  - [x] 申請編集画面にファイル形式・サイズのバリデーションを追加（`application-edit.component.ts`）
  - [x] 社員情報編集画面にファイル形式・サイズのバリデーションを追加（`employee-edit.component.ts`）
  - [x] 組織の`attachmentSettings`または`documentSettings`から許可形式・最大サイズを取得（設定がない場合はStorageルールのデフォルト値を使用）
  - [x] ファイルアップロード時のエラーハンドリングを追加（Storageルールエラーを適切に処理）

#### 【修正26】申請管理の複数ファイル添付とプレビュー機能の追加
- [x] **実装完了**
  - [x] 申請作成画面の`onFileSelected`メソッドを修正（既存ファイルに追加、重複チェック、確認ダイアログ）
  - [x] 申請編集画面の`onFileSelected`メソッドを修正（既存ファイルに追加、重複チェック、確認ダイアログ、`existingAttachments`も考慮）
  - [x] 重複ファイル名の確認ダイアログを追加（OKで上書き、キャンセルで追加しない）
  - [x] 申請作成画面の内容確認ステップにプレビュー機能を追加（新規ファイルのプレビュー）
  - [x] 申請編集画面の内容確認ステップにプレビュー機能を追加（既存ファイルと新規ファイルの両方に対応）
  - [x] ファイルプレビュー用URLのメモリ管理（`ngOnDestroy`でクリーンアップ）

#### 【修正27】社員情報編集の複数ファイル添付とプレビュー機能の追加
- [x] **実装完了**
  - [x] 社員情報編集画面の`onFileSelected`メソッドを修正（既存ファイルに追加、重複チェック、確認ダイアログ、`existingAttachments`も考慮）
  - [x] 重複ファイル名の確認ダイアログを追加（OKで上書き、キャンセルで追加しない）
  - [x] 既存ファイル削除の戻す機能を追加（`cancelDeleteExistingFile`、`isExistingFileDeleted`メソッド）
  - [x] 社員情報編集画面にプレビュー機能を追加（既存ファイルと新規ファイルの両方に対応）
  - [x] ファイルプレビュー用URLのメモリ管理（`ngOnDestroy`でクリーンアップ）

#### 【修正20】通知機能の削除とダッシュボード統合
- [x] **通知送信機能の無効化**
  - [x] `NotificationService`の通知送信メソッドをコメントアウト（早期リターンを追加）
  - [x] 通知送信機能の役割をダッシュボード表示に統合
  - [x] 通知関連のメソッドに早期リターンを追加して機能を無効化
    - [x] `checkAndSendDeadlineReminders`
    - [x] `checkAndSendStandardRewardReminders`
    - [x] `checkAndSendMonthlyChangeReminders`
    - [x] `checkAndSendMonthlyCalculationReminders`
    - [x] `checkOverdueNotificationsWithoutApplication`
    - [x] `sendDeadlineReminder`
    - [x] `sendImmediateDeadlineReminderIfNeeded`
    - [x] `createApplicationStatusNotification`
    - [x] `checkAndSendRewardChangeNotifications`
    - [x] `checkAndSendAddressAndNameChangeNotifications`
- [x] **ダッシュボードへの統合**
  - [x] 要対応サマリーに未提出申請を申請種別ごとに表示
  - [x] 期限・リマインダーに期限超過・期限5日以内を申請種別ごとに表示
  - [x] 最近の動きに申請ステータス変更を追加（直近5件）
  - [x] 申請一覧への遷移時に申請種別のみでフィルタリング
...
- [x] 期限リマインダー機能
  ...
  - [x] 重複チェックの改善
    ...
    - [x] 全通知タイプの重複チェック改善（修正19）
      - [x] 期限超過通知（申請がある場合）：`sendDeadlineReminder`内で、期限超過通知の場合も各ユーザーごとに今日の日付で重複チェックを実装、`checkAndSendDeadlineReminders`の事前チェック（`adminUids[0]`のみ）を削除
      - [x] 申請がない場合の期限超過通知：各ユーザーごとに今日の日付で重複チェックを実装（事前チェックを削除）
      - [x] 報酬月額変更届の通知：各ユーザーごとに今日の日付で重複チェックを実装（事前チェックを削除）
      - [x] 住所変更届・氏名変更届の通知：各ユーザーごとに今日の日付で重複チェックを実装（事前チェックを削除）
      - [x] 算定計算のリマインダー通知：`createStandardRewardCalculationReminder`内で各ユーザーごとに今日の日付で重複チェックを実装（事前チェックを削除）
      - [x] 月変計算のリマインダー通知：`createMonthlyChangeCalculationReminder`内で各ユーザーごとに同じ年月で重複チェックを実装（事前チェックを削除）
  ...
  - [x] **通知送信の問題対応（修正16・修正18）**
    - [x] 法定期限リマインダー通知が来ない原因を調査・修正
    - [x] `checkAndSendDeadlineReminders`から報酬月額変更届・住所変更届・氏名変更届を除外（期限計算を行わないため）
    - [x] `checkOverdueNotificationsWithoutApplication`から報酬月額変更届・住所変更届・氏名変更届を除外（期限計算を行わないため）
    - [x] 報酬月額変更届の通知ロジックを変更（期限計算なし、月変計算確定後に対象者通知を1日1回送信）
      - [x] `checkAndSendRewardChangeNotifications`メソッドを追加
      - [x] 月変計算確定済み（status: 'applied'）で、外部申請が作成されていない社員に対して通知を送信
    - [x] 住所変更届・氏名変更届の通知ロジックを変更（期限計算なし、内部申請作成後1日1回通知）
      - [x] `checkAndSendAddressAndNameChangeNotifications`メソッドを追加
      - [x] 内部申請が作成されたら、関連する外部申請が作成されない限り1日1回通知を送信
    - [x] `setDeadlineAndSendReminderForInternalApplication`で住所変更届・氏名変更届を除外（通知は別メソッドで送信）
  - [x] **通知設定の改善（修正19）**
    - [x] 初回セットアップに`reminderSettings`セクションを追加
      - [x] `internalDeadlineDays`、`externalDeadlineDays`、`reminderInterval`を削除
      - [x] `reminderSettings`（`adminDaysBeforeLegalDeadline`、`notifyOnOverdue`、`notifyOnDeadlineDay`、`notifyBeforeDeadline`）を追加
    - [x] 申請作成・編集時の期限設定ロジックを変更
      - [x] 外部申請：法定期限がない場合は期限設定なし
      - [x] 内部申請：期限設定なし
    - [x] データモデルの変更（`application-flow.model.ts`）
      - [x] `internalDeadlineDays`、`externalDeadlineDays`、`reminderInterval`をオプショナル化（後方互換性のため）
      - [x] `reminderSettings`を必須化
    - [ ] 賞与支払届の通知対応（給与情報入力画面での賞与支払日設定機能実装後に修正）
      - [ ] `BonusData`から支払日を取得して期限計算を行うように修正
      - [ ] 期限リマインダー通知の送信ロジックを実装
...
- [x] 組織情報編集
  - [x] 組織名、法人番号、事業所番号の編集
  - [x] **事業所整理記号の追加**
    - [x] `Organization`モデルに`officeSymbol`フィールドを追加
    - [x] 組織設定画面に事業所整理記号入力欄を追加（事業所番号の上に配置）
    - [x] 初回セットアップ画面にも事業所整理記号入力欄を追加
    - [x] 各申請フォームの事業所整理記号に組織設定の値を自動設定（ユーザーが上書き可能）
  - [x] 住所情報の編集（都道府県、市区町村、**町名・番地**、建物名）
    - [x] 「番地」→「町名・番地」に表示ラベルを変更
...
- [x] 料率設定（過去・現在・将来）
  ...
  - [x] 保険料率テーブルの削除機能
  - [x] **適用年フィルタの改善**
    - [x] 終了日未設定の等級表がある場合、現在年+2年まで表示されるように修正
    - [x] 保険料計算ダイアログも未来2年まで選択可能に修正
    - [x] フィルタリングのヒントテキストを追加（「終了月未設定の等級表は現在年＋2年まで表示されます」）
- [x] 標準報酬上限・下限設定
  - [x] 標準報酬月額の設定（等級ごと）
  - [x] 最小値・最大値の設定
- [x] CSV/Excelインポート
  ...
  - [x] **厚生年金料率表の自動補完ロジック修正**
    - [x] 問題：1級より上の空欄に最多級の情報が入ってしまうバグを修正
    - [x] 解決策：健保等級（grade）でソートし、位置ベースで判定する方法に変更
    - [x] 実装箇所：`setup-wizard.component.ts`、`insurance-rate-table-manager-dialog.component.ts`
  - [x] **新規追加時の「既存を上書き」機能の修正**
    - [x] 問題：ケース3（適用開始月が同じ場合）で、該当テーブル以外の既存テーブルも削除されてしまう
    - [x] 解決策：適用開始月が同じ既存テーブルのみを削除するように修正
    - [x] 確認：ケース2-2（新規終了月未設定）は問題なし（新規開始月以降のすべてのテーブルを削除するのは意図通り）

#### 8.2.1 保険料・標準報酬設定の改善候補（未実装）
- [ ] **保険料率テーブルのバリデーション強化**
  - [ ] 同じ等級・適用期間の重複チェック（新規追加時）
  - [ ] 最小値・最大値の整合性チェック（minAmount <= maxAmount）
  - [ ] 標準報酬月額が最小値・最大値の範囲内かチェック
- [ ] **UI/UXの改善**
  - [ ] エラーメッセージの詳細化（どの行でエラーが発生したか明確に表示）
  - [ ] インポート時の進捗表示（大量データ対応）
  - [ ] 適用期間の重複警告の改善（視覚的に分かりやすく）
- [ ] **将来計算の動作確認・テスト**
  - [ ] 終了日未設定の等級表での将来計算（現在年+2年）の動作確認
  - [ ] 適用期間が重複する場合の計算ロジック確認
- [ ] **パフォーマンス改善**
  - [ ] 大量の等級表データの処理最適化
  - [ ] フィルタリング処理の最適化
...
- [x] リマインダー設定
  ...
  - [x] 期限超過、当日、事前通知の設定（設定可能）
- [x] 月次計算日の通知設定
  - [x] 組織情報に月次計算予定日を設定する欄を追加（「給与支払日」→「月次計算予定日」に名称変更、1-31日のドロップダウン選択、29-31日は存在しない月は月末に自動調整）
  - [x] 月次計算予定日になったら通知を送信する機能（ダッシュボード読み込み時にチェック）
  - [x] 未計算の社員がいる場合のみ通知を送信
  - [x] 各ユーザーごとの重複チェック実装（`createMonthlyCalculationReminder`内で各`adminUid`ごとに重複チェックを実行）
  - [x] 月次計算通知の対象月設定（当月/翌月モード）
    - [x] `Organization`モデルに`monthlyCalculationTargetMonth`フィールドを追加（'current' | 'next'、デフォルト: 'current'）
    - [x] 組織設定画面に「翌月の計算を促す」チェックボックスを追加
    - [x] 翌月モードの場合、1ヶ月前の計算を促す通知を送信（例: 12月12日なら11月分の計算を促す）
...
#### 9.1 保険料集計機能
- [x] 社員別集計（選択年月の確定済み・出力済み計算結果から集計）
- [x] 部署別集計（社員別集計を部署ごとに集計）
- [x] 会社全体集計（全社員の合計）
- [x] 年月選択機能（年・月のドロップダウン）

#### 9.2 月次推移機能
- [x] 選択年の1月から12月までの月次推移を表示
- [x] 被保険者負担額合計、事業主負担額合計、合計保険料を表示
- [x] 年選択機能（ドロップダウン）

#### 9.3 算定・月変の影響確認機能
- [ ] 算定計算の影響確認（前年の9月時点と算定後の7月時点の標準報酬・保険料を比較）
- [ ] 月変計算の影響確認（変動月の前月と変動月から4か月目の標準報酬・保険料を比較）
- [ ] 標準報酬月額の前後比較と差額表示
- [ ] 事業主負担額の前後比較と差額表示
- [ ] 影響サマリー表示（変更人数、事業主負担額の増減合計）
- [ ] 年選択機能（ドロップダウン）
- **注意**: 後で追加する可能性があるため、現在はコメントアウト済み

#### 9.4 CSV出力機能
- [x] 月次保険料一覧のCSV出力（社員別集計）
- [x] 部署別集計のCSV出力（会社合計行を含む）
- [x] 月次推移のCSV出力
- [ ] 算定・月変の影響確認のCSV出力（算定計算と月変計算を分けて出力）- **注意**: 9.3と同様、後で追加する可能性があるため現在はコメントアウト済み

#### 9.5 UI実装
- [x] タブ形式のUI（保険料集計、月次推移）- 算定・月変の影響確認タブは後で追加する可能性があるため現在はコメントアウト
- [x] Materialテーブルでのデータ表示
- [x] 通貨フォーマット表示（¥記号付き、カンマ区切り）
- [x] 増減額の色分け表示（増加：赤、減少：青）
- [x] ローディング表示
- [x] 会社全体集計の説明追加（被保険者負担額合計と事業主負担額合計は、保険料計算結果から取得した各社員の負担額（折半額）を合計した値であることを明記）

#### 9.6 ルーティング・ナビゲーション
- [x] `/analytics`ルートの追加
- [x] サイドバーに「分析」メニューを追加（管理者モードのみ）
- **注意**: ダッシュボードには分析機能へのリンクやサマリーは追加していない（サイドバーのメニューからアクセス）

**注意**: 
- 申請種別別集計と申請件数の推移は不要（要件により除外）。保険料の集計と月次推移に特化した実装。
- 算定・月変の影響確認機能は後で追加する可能性があるため、コードはコメントアウト済み。
...
#### 11.1 情報表示
- [x] 基本情報表示
  - [x] 社員番号、氏名、氏名カナ、メールアドレス、部署、入社日、生年月日、ステータス
  - [x] 基本情報は編集不可（申請または管理者へのシステム外の申し出で変更）
- [x] 住所情報表示（正式住所のみ）
  - [x] 正式住所の表示
  - [x] 正式住所の変更には申請が必要である旨の注意書き
- [x] 扶養情報表示
  - [x] 扶養者の一覧表示
  - [x] 扶養情報の変更には申請が必要である旨の注意書き
- [x] 保険情報表示
  - [x] 健康保険被保険者番号、厚生年金被保険者番号、マイナンバー、保険適用開始日
  - [x] 平均報酬月額、等級、標準報酬月額、等級適用年月日は表示しない（削除済み）
- [x] 他社勤務情報表示
- [x] ルーティング追加（`/my-info`）
- [x] ヘッダーからの遷移機能
...
#### 11.2 情報編集
- [x] 基本情報は編集不可（申請または管理者へのシステム外の申し出で変更）
- [ ] 連絡先情報の編集（余裕があったら最後に追加）
  - [ ] 電話番号の編集
  - [ ] 緊急連絡先情報の編集（氏名、続柄、電話番号）

#### 11.3 申請作成
- [x] 申請ステータスタブは削除（自分の申請画面と被るため）
- [x] 「申請を作成」ボタンは削除（自分の申請画面と被るため）
- [x] 保険料タブは削除

#### 11.4 アカウント設定画面
- [x] ルーティング追加（`/account-settings`）
- [x] ヘッダーからの遷移機能
- [x] パスワード変更機能
  - [x] 現在のパスワード確認
  - [x] 新しいパスワード設定（8文字以上）
...
- [x] 通知設定
  - [x] メール通知のON/OFF
  - [x] アプリ内通知のON/OFF
  - [x] Userモデルに通知設定フィールド追加（`emailNotificationEnabled`, `inAppNotificationEnabled`）
  - [x] AuthServiceに`updateUserSettings()`メソッド追加

**注意**: 
- 通知関係の表示などの細かい修正は、通知画面の表示方法（申請ごとにまとめるかどうかなど）が確定してから実施予定。
- 通知のリマインダー機能のCloud Functionsへの移行は、ダッシュボード実装完了後に実施予定（現在はダッシュボード読み込み時にチェックする形で実装済み）。
...
#### 10.1 データインポート
- [x] CSV/Excelインポート機能
  - [x] 社員情報インポート（他社勤務有無、主たる勤務先、会社名の列を削除）
  - [x] ヘッダー行の解析機能（ヘッダー行から列インデックスをマッピング）
  - [x] 「被保険者整理番号」「基礎年金番号」をヘッダー行から探してマッピング
  - [x] サンプルデータのCSVテキストとプレビューテーブルのラベルを「被保険者整理番号」「基礎年金番号」に変更
  - [x] 給与情報インポート
  - [x] 給与情報インポートにサンプルデータ機能追加（CSV/Excelダウンロード）
- [x] データ競合検出
- [x] 差分表示・選択機能
- [x] **競合解決機能の削除**
  - [x] 外部連携画面の社員情報インポート機能を社員情報画面のインポート機能と同じ実装に統一
  - [x] 既存社員はエラーとして扱い、競合解決機能を削除
  - [x] 選択チェックボックス機能を削除（エラーがないデータはすべてインポート）
  - [x] 競合解決関連のコードを完全削除（コメントアウトではなく削除）
  - [x] 不要なインポート（`MatCheckboxModule`, `MatRadioModule`, `MatTooltipModule`, `ConflictResolutionDialogComponent`）を削除

#### 10.2 データエクスポート
- [x] 社員選択機能
- [x] 出力データ種別選択
  - [x] 基本情報、保険料情報（月次）、保険料情報（賞与）、給与情報の選択機能
  - [x] 保険料情報（月次）と保険料情報（賞与）をデフォルトで選択状態に設定
- [x] CSV/Excel出力
  - [x] 保険料情報（月次）のエクスポート
  - [x] 保険料情報（賞与）のエクスポート
  - [x] 給与情報のエクスポート
...
**実装タイミング**: フェーズ11（自分の情報画面）→ フェーズ10（外部連携）→ ダッシュボード実装 → Cloud Functions移行 → フェーズ12（テスト・最適化）の順序で実施

#### ダッシュボード要件（最低限構成）
- [x] **必須①：要対応サマリー**
  - [x] 未承認の内部申請：◯件（クリック可能で該当一覧へ遷移）
  - [x] 計算未確定の保険料：◯件（クリック可能で該当一覧へ遷移）
  - [x] **【修正20】未提出申請を申請種別ごとに表示**
    - [x] 申請種別ごとに未提出申請（draft, created, returned）の件数を表示
    - [x] クリックで申請一覧に遷移（申請種別のみフィルタ）
- [x] **必須②：期限・リマインダー系**
  - [x] **【修正20】申請種別ごとの期限超過・期限5日以内件数表示**
    - [x] 申請種別ごとに期限超過件数と期限5日以内件数を表示
    - [x] クリックで申請一覧に遷移（申請種別のみフィルタ）
  - [x] 直近の期限アラート表示
  - [x] 被保険者資格取得届などの期限表示
  - [x] 月額変更届対象者ありの表示
- [x] **必須③：最近の動き（変更ログ要約）**
  ...
  - [x] 標準報酬月額変更の表示
  - [x] 保険料計算の確定の表示
  - [x] **【修正20】申請ステータス変更の表示（直近5件）**
    - [x] 申請のステータス変更（送信、承認、却下、差戻し、取り下げ）を表示
    - [x] 申請種別名、社員名、ステータス変更内容を含むメッセージ
  - [x] 詳細ログ画面へのリンク
- [x] **任意：今月の概況（軽め）**
  - [x] 今月の被保険者数
  - [x] 今月の保険料合計（概算）
- [x] **レイアウト統一**
  - [x] ダッシュボード、分析、設定ページのレイアウトを他のページと統一（h1タグ削除、mat-cardで囲む）
  - [x] ダッシュボードのロール表示を削除
- [x] **【修正21】一般社員画面のダッシュボード**
  - [x] 管理者モードと社員モードでダッシュボードの表示を分岐
  - [x] **自分の申請状況セクション**
    - [x] 自分の内部申請のみを表示（employeeIdでフィルタ、category: 'internal'）
    - [x] 申請種別ごとにグループ化
    - [x] 各申請種別でステータス別の件数を表示
    - [x] 直近5件の申請を表示（申請種別×ステータスの組み合わせで最大5件）
    - [x] クリックで申請一覧に遷移（該当の申請種別とステータスでフィルタ）
  - [x] **申請作成のショートカットセクション**
    - [x] 内部申請種別のみを表示（被扶養者（異動）届、住所変更届、氏名変更届の3種類のみ）
    - [x] 各申請種別に「作成」ボタンを配置
    - [x] クリックで申請作成画面に遷移（申請種別を指定）
  - [x] 組織情報の読み込み完了を待ってからダッシュボードデータを読み込むように修正（タイミング問題の解決）

**注意**: 
- ダッシュボードは「状況把握と導線」が主目的。分析・比較・CSV出力は分析画面で実施。
- 部署別保険料詳細、社員別一覧、月次推移グラフ、算定・月変の影響比較はダッシュボードには不要（分析ページの仕事）。
- 通知関係の細かい修正は、通知画面の表示方法（申請ごとにまとめるかどうかなど）が確定してから実施する。

---

**注意**: 通知機能を諦めたため、Cloud Functions移行は不要となりました。
...
- [x] **Firestoreセキュリティルール**
  - [x] 組織単位でのアクセス制御
  - [x] 権限に応じた読み書き制限（owner/admin/employee）
  - [x] 既存アプリとの完全分離（socialInsurance_プレフィックスのみ制御）
  - [x] コレクションごとの細かい制御
    - [x] socialInsurance_organizations: 組織のメンバーのみ読み取り、ownerのみ更新・削除
    - [x] socialInsurance_users: 自分のデータまたは管理者（同じ組織内）
    - [x] socialInsurance_employees: 管理者は全社員、社員は自分のデータのみ
    - [x] socialInsurance_departments: 組織のメンバーのみ読み取り、管理者のみ書き込み
    - [x] socialInsurance_applications: 管理者は全申請、社員は自分の申請のみ
    - [x] socialInsurance_calculations: 管理者は全データ、社員は自分のデータのみ
    - [x] socialInsurance_insuranceRates: 組織のメンバーのみ読み取り、管理者のみ書き込み
    - [x] socialInsurance_notifications: 自分の通知のみ読み書き
    - [x] socialInsurance_documents: 管理者は全データ、社員は自分のデータのみ
    - [x] socialInsurance_standardRewards: 組織のメンバーのみ読み取り、管理者のみ書き込み
    - [x] socialInsurance_settings: 組織のメンバーのみ読み取り、管理者のみ書き込み
- [x] **Storageセキュリティルール**
  - [x] 組織単位でのアクセス制御
  - [x] ファイルサイズ・形式制限
    - [x] 申請添付書類: 最大50MB、PDF/画像/Excel/Word形式（アプリ側の設定に合わせて更新）
    - [x] 社員ファイル: 最大50MB、PDF/画像/Excel/Word形式（アプリ側の設定に合わせて更新）
    - [x] 組織ロゴ: 最大5MB、画像形式のみ
    - [x] テンプレート: 最大10MB、CSV/Excel/PDF形式
  - [x] 既存アプリとの完全分離（social-insurance/パスのみ制御）
  - [x] パスごとの細かい制御
    - [x] organizations/{orgId}/documents/: 組織のメンバーのみ
    - [x] organizations/{orgId}/employees/: 管理者は全データ、社員は自分のデータのみ
    - [x] organizations/{orgId}/logos/: 組織のメンバーのみ読み取り、管理者のみ書き込み
    - [x] templates/: 認証済みユーザー全員読み取り、管理者のみ書き込み
    - [x] Storageルールの更新：申請添付書類と社員ファイルでExcel/Word形式も許可（アプリ側の設定に合わせて）
- [ ] 認証チェック強化
  - [x] 各API呼び出しでの認証チェック（既存実装）
  - [x] 権限チェックの徹底（既存実装）
  - [ ] セッション管理（必要に応じて）
- [ ] データバリデーション
  - [x] クライアント側バリデーション（既存実装）
  - [x] サーバー側（Firestore Rules）バリデーション（セキュリティルールで実装）

**セキュリティルールのテスト・デプロイ手順**:
- [x] **エミュレーターでの動作確認**
  - [x] セキュリティルールファイルの作成（firestore.rules, storage.rules）
  - [x] firebase.jsonの設定
  - [ ] エミュレーターでのテスト実施
    - [ ] エミュレーター再起動（セキュリティルール変更時は必須）
    - [ ] テストデータの準備（複数組織、管理者・社員アカウント）
    - [ ] **最優先テスト項目**
      - [ ] 組織間のデータ漏洩防止（他の組織のデータにアクセスできないか）→動作確認大丈夫だと思います
      - [ ] 権限による制御（社員が管理者権限で操作できないか）→社員のサイドバーだし申請も内部申請しか作成できない、外部申請は見れてない、大丈夫かと。
      - [ ] 認証チェック（未認証ユーザーがアクセスできないか）→社員登録してもメール送信してないのでパスワードが不明なのでログイン不可。ユーザーが見つかりませんエラーも表示
    - [ ] **主要コレクションのテスト**
      - [ ] socialInsurance_applications: 作成・読み取り・更新・削除→権限による制御は大丈夫そう
      - [ ] socialInsurance_employees: 管理者は全社員、社員は自分のみ→権限による制御は大丈夫そう
      - [ ] socialInsurance_users: 自分のデータのみアクセス可能
    - [ ] **Storageのテスト**
      - [ ] 申請添付書類のアップロード・読み取り
      - [ ] ファイルサイズ制限（50MB超で拒否）
      - [ ] ファイル形式制限（PDF/画像/Excel/Word）→Storageルールを更新してExcel/Word形式も許可
      - [ ] 組織ロゴのアップロード（管理者のみ）
      - [ ] エミュレーターでのルール適用確認（Storage Emulatorはルールファイルの変更を自動検知して更新）
    - [ ] **追加の動作確認項目**
      - [x] **オーナー権限での外部申請作成・編集**
        - [x] オーナーアカウント（employeeIdがnull）で外部申請を作成できるか→動作確認完了
        - [x] オーナーアカウントで外部申請を編集できるか→動作確認完了
        - [x] オーナーアカウントで外部申請の添付ファイル（Excel/Word）をアップロードできるか→動作確認完了
      - [ ] **メール認証チェック**
        - [x] メール認証未完了のユーザーが組織作成画面にアクセスできないか（/email-verificationにリダイレクトされるか）→修正完了
        - [x] メール認証未完了かつ組織未作成のユーザーがログイン後、ダッシュボードにアクセスしようとした場合、/email-verificationにリダイレクトされるか（修正23の追加実装）→動作確認完了
        - [x] メール認証未完了のユーザーがセットアップ画面にアクセスできないか（/email-verificationにリダイレクトされるか）→動作確認完了
        - [x] メール認証完了後に組織作成画面にアクセスできるか→動作確認完了
        - [x] 社員（organizationIdが既に設定されている）は、メール認証未完了でもダッシュボードにアクセスできるか（影響がないことを確認）→動作確認完了（パスワード未設定となるため考慮不要）
      - [x] **Storageのセキュリティルール詳細確認**
        - [x] 許可されたファイル形式（PDF/画像/Excel/Word）がアップロードできるか→動作確認完了
        - [x] 許可されていないファイル形式（例：.txt、.zip）が拒否されるか（エラーメッセージが表示されるか）→動作確認完了（クライアント側バリデーションで拒否＆エラー表示）
        - [x] 50MB超のファイルが拒否されるか（エラーメッセージが表示されるか）→動作確認完了（クライアント側バリデーションで拒否＆エラー表示）
        - [x] 他の組織のファイルにアクセスできないか（読み取り・書き込み・削除）→動作確認完了
        - [x] 社員が自分のファイルのみアクセスできるか（社員ファイルのパス）→動作確認完了（社員がファイルを添付できるのは申請のみ）
        - [x] 管理者が全社員のファイルにアクセスできるか→動作確認完了
  - [x] **基本的な動作確認完了**（Emulator環境）
    - [x] オーナー権限での外部申請作成・編集
    - [x] メール認証チェック（組織作成画面・セットアップ画面へのアクセス制御、ログイン後のリダイレクト）
    - [x] Storageのセキュリティルール（ファイル形式・サイズ制限、他組織へのアクセス制御）
    - [x] クライアント側ファイルバリデーション（申請作成・編集、社員情報編集）

  - [ ] **ルール関係以外の修正項目**（順次実施）
    - [ ] **優先度：高** - 動作確認で発見された問題の修正
      - [ ] バグ修正（動作確認で発見された問題）
      - [ ] エラーハンドリングの改善
      - [ ] データ整合性の問題修正
    - [ ] **優先度：中** - UI/UXの改善
      ...
      - [ ] エラーメッセージの改善
      - [ ] ローディング状態の改善
    - [ ] **優先度：低** - パフォーマンス最適化・その他
      - [ ] パフォーマンス最適化
      - [ ] コードのリファクタリング
      - [ ] ドキュメントの整備

  - [ ] エミュレーターでの動作確認完了後、本番環境にデプロイ
    - [ ] `firebase deploy --only firestore:rules` でFirestoreルールをデプロイ
    - [ ] `firebase deploy --only storage` でStorageルールをデプロイ
    - [ ] 本番環境でも動作確認

**注意事項**:
- Firebase EmulatorのStorageはルールファイルの変更を自動検知して更新する（`Change detected, updating rules for Cloud Storage...`メッセージが表示される）
- Firestore Emulatorはルールファイルの変更を自動検知しないため、変更時はエミュレーターを再起動する必要がある
- エミュレーターで十分にテストしてから本番環境にデプロイすること
- デプロイ後も本番環境で動作確認を行うこと

**今後の進め方**:
1. **Emulator環境での動作確認**（完了）
   - 基本的な動作確認は完了
   - ルール関係以外の修正項目を順次実施
2. **ルール関係以外の修正**（進行中）
   - 動作確認で発見された問題を優先的に修正
   - UI/UXの改善
   - パフォーマンス最適化
3. **ルールのデプロイ**（Hosting設定前でも可能）
   - Firestoreルールをデプロイ
   - Storageルールをデプロイ
4. **Hosting設定とアプリのデプロイ**（最後）
   - Firebase Hostingの設定
   - Angularアプリのビルドとデプロイ
   - 本番環境での最終動作確認
...
socialInsurance_organizations/          # 組織（企業）
  {orgId}/
    ...
    - officeSymbol: string              # 事業所整理記号（任意）
...
### Organizationモデルの変更（フェーズ8実装時）
- **事業所整理記号の追加**
  - `Organization`モデルに`officeSymbol?: string`フィールドを追加
  - 組織設定画面と初回セットアップ画面に事業所整理記号入力欄を追加
  - 各申請フォーム（被保険者資格取得届、被保険者資格喪失届、被扶養者（異動）届、被保険者住所変更届、被保険者氏名変更届、標準報酬決定・改定届、標準報酬月額変更届、被保険者賞与支払届）の事業所整理記号に組織設定の値を自動設定
  - ユーザーは申請フォームで事業所整理記号を上書き可能
- **住所フィールドの表示ラベル変更**
  - 「番地」→「町名・番地」に表示ラベルを変更（組織設定画面、初回セットアップ画面）
  - エラーメッセージも「町名・番地は必須です」に変更
...
#### 各被保険者ごとの期限計算対応
- **資格取得届・資格喪失届・賞与支払届**: 各被保険者ごとに期限を計算し、`data.insuredPersons[].deadline`に保存
- **資格取得届**: `acquisitionDate + 5日`で各被保険者の期限を計算
- **資格喪失届**: `lossDate + 5日`で各被保険者の期限を計算（従来の`retirementDate + 6日`から変更）
- **賞与支払届**: 各被保険者の`bonusPaymentDate`（優先）または`commonBonusPaymentDate`から`支払日 + 5日`で期限を計算
- **報酬月額変更届**: 期限なし（速やかに届け出るため）
- **実装ファイル**: `src/app/core/services/deadline-calculation.service.ts`

#### 申請一覧の表示改善
- **列構成の変更**: `['type', 'applicant', 'targetEmployees', 'category', 'status', 'createdAt', 'deadlineStatus', 'actions']`
- **申請者表示**: `getApplicantName()`メソッドで、`employeeId`が`undefined`の場合は会社名を表示（オーナーアカウント対応）
- **対象社員表示**: 
  - 1名: 「社員A名」
  - 2名: 「社員A名、社員B名」
  - 3名以上: 「社員A名　他〇名」
  - 手入力時は被保険者情報から氏名を取得
- **期限ステータス表示**: 「期限超過：●名、5日以内：●名」の形式で集計表示
- **実装ファイル**: `src/app/features/applications/application-list/application-list.component.ts`

#### 申請詳細の期限表示改善
- **ヘッダーに期限情報を表示**: 各被保険者ごとの期限をチップ形式で表示（色分け・アイコン付き）
  - 期限超過: 赤色チップ + errorアイコン
  - 5日以内: オレンジ色チップ + warningアイコン
  - 通常: 青色チップ + scheduleアイコン
- **各被保険者情報の期限を強調表示**: 
  - 期限超過: 赤のボーダー + 赤背景 + 「期限超過」バッジ
  - 5日以内: オレンジのボーダー + オレンジ背景 + 「5日以内」バッジ
  - 通常: 青のボーダー
- **被扶養者異動届の期限表示**: 申請全体の期限を「期限情報」セクションとして表示
- **実装ファイル**: 
  ...
  - `src/app/features/applications/application-detail/application-detail.component.css`

#### フィールド名の統合
- **賞与支払届のフィールド名統一**:
  - `commonPaymentDate` → `commonBonusPaymentDate`（年号形式）
  - `paymentDate` → `bonusPaymentDate`（年号形式）
  - 保存時にDate形式に変換して保存
- **実装ファイル**: 
  - `src/app/features/applications/application-create/application-create.component.ts`
  - `src/app/features/applications/application-create/application-create.component.html`
  - `src/app/features/applications/application-edit/application-edit.component.ts`
  - `src/app/features/applications/application-edit/application-edit.component.html`

#### 申請者表示の統一
- **申請一覧と申請詳細で同じロジック**: `getApplicantName()`メソッドで統一
- `employeeId`がある場合は社員名、`undefined`の場合は会社名を表示
- **実装ファイル**: 
  - `src/app/features/applications/application-list/application-list.component.ts`
  - `src/app/features/applications/application-detail/application-detail.component.ts`

#### 日付自動転記機能
- **資格取得届**: 社員選択時に`employee.joinDate`を`acquisitionDate`に自動転記
- **資格喪失届**: 社員選択時に`employee.retirementDate + 1日`を`lossDate`に自動転記
- **実装ファイル**: 
  - `src/app/features/applications/application-create/application-create.component.ts`
  - `src/app/features/applications/application-edit/application-edit.component.ts`

#### ダッシュボードの要対応サマリー修正
- **未提出申請のフィルタリング**: 管理者モードの場合、内部申請の`draft`/`created`状態を除外（申請一覧と同じロジック）
- **実装ファイル**: `src/app/features/dashboard/admin-dashboard/admin-dashboard.component.ts`

#### 社員情報の変更履歴機能
- **変更履歴の記録条件**: メール認証後（`emailVerified === true`）の編集のみ変更履歴に記録
- **削除可能な社員**: 変更履歴に記録しない
- **添付ファイルの変更履歴**: 追加・削除を個別に記録
- **保存時の確認ダイアログ**: メール認証後の編集で変更がある場合のみ表示
- **変更履歴の表示改善**: 
  - 変更箇所だけを1行で分かりやすく表示
  - 例: `氏: 山田 → 田中`、`添付ファイル「ファイル1.pdf」を追加`
  - 保険情報の各フィールドを個別に表示（例: `等級の適用開始日: - → 2024/01/01`）
- **実装ファイル**: 
  - `src/app/features/employees/employee-edit/employee-edit.component.ts`
  - `src/app/features/employees/employee-detail/employee-detail.component.ts`
  - `src/app/features/employees/employee-detail/employee-detail.component.html`
  - `src/app/features/employees/employee-detail/employee-detail.component.css`

#### 申請フロー設定の改善
- **申請種別の制限**: 
  - 外部申請：8種類のみ表示可能（`INSURANCE_ACQUISITION`, `INSURANCE_LOSS`, `DEPENDENT_CHANGE_EXTERNAL`, `REWARD_BASE`, `REWARD_CHANGE`, `ADDRESS_CHANGE_EXTERNAL`, `NAME_CHANGE_EXTERNAL`, `BONUS_PAYMENT`）
  - 内部申請：3種類のみ表示可能（`DEPENDENT_CHANGE`, `NAME_CHANGE`, `ADDRESS_CHANGE`）
- **承認ルール設定タブ**: コメントアウト（不要のため）
- **添付書類設定のプルダウン**: 指定した申請種別以外は非表示
- **申請種別の編集**: 
  - コードの変更は不可（編集時は変更不可）
  - 説明文フィールドを削除
  - 申請種別コードの表示を削除
- **実装ファイル**: 
  - `src/app/features/settings/application-flow-settings/application-flow-settings.component.ts`
  - `src/app/features/settings/application-flow-settings/application-flow-settings.component.html`
  - `src/app/features/settings/application-flow-settings/application-type-edit-dialog/application-type-edit-dialog.component.html`
  - `src/app/features/settings/application-flow-settings/attachment-setting-edit-dialog/attachment-setting-edit-dialog.component.ts`
  - `src/app/features/setup/setup-wizard/setup-wizard.component.ts`
  - `src/app/features/setup/setup-wizard/setup-wizard.component.html`

#### 最終確認画面の改善
- **ステップ1の変更**: 
  - 事業所番号の表示を削除
  - 住所に郵便番号を含める（`〒{postalCode} {address}`形式）
  - 事業主氏名を追加
- **実装ファイル**: 
  - `src/app/features/setup/setup-wizard/setup-wizard.component.ts`
  - `src/app/features/setup/setup-wizard/setup-wizard.component.html`
...
#### 部署設定の改善
- **上位部署フィールド**: コメントアウト（不要のため）
- **実装ファイル**: 
  - `src/app/features/settings/department-settings/department-settings.component.html`
  - `src/app/features/setup/setup-wizard/setup-wizard.component.html`
